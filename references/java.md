...
步骤5默认使用自包含 SVG 绘制流程图，并保存为 `control_flow.svg`，在 `path_analysis.md` 中使用 `![控制流程图](control_flow.svg)` 引用。使用圆角矩形表示开始/结束，菱形表示判定，矩形表示处理步骤，带箭头的 `<path>` 表示控制流；循环必须有回箭头，所有判定和真/假分支都要标注。SVG 应包含 `viewBox`、`<title>` 和 `<desc>`，且不依赖外部渲染器、脚本或样式表。
